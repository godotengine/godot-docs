...
top-left and double-click on *Node*. Name the node ``Main``. An alternate method to rename the node is to right-click on the node and choose :menuselection:`Rename` (or :kbd:`F2`). Alternatively, to add
