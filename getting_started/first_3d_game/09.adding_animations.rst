...
animation. Next, click on **Edit (Next to A+ button) > Copy Tracks**. 

Then open ``mob.tscn``, create an AnimationPlayer child node and select it. In the animation dock below, create new animation with **Animation > New**, and name it ``float_mob``. 
Now for the magic, click **Edit > Paste Tracks**.

Enable "A+" Autoplay on Load icon, and the looping arrows (Animation looping) in the animation editor just below. 
The animation duration should be set to ``1.2``, same as previous ``float``.
...
.. |image23| image:: img/09.adding_animations/22.update_tracks.png
