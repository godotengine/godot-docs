...
To continuously play background music, we introduce the :ref:`Autoloads <autoloads_versus_internal_nodes>` function.

To play audio, all you need to do is add an :ref:`AudioStreamPlayer <class_AudioStreamPlayer>` 
node to your scene and attach an audio file to it. When you start the scene, it can play
...
With the :ref:`Autoloads <doc_singletons_autoload>` feature, Godot can load a ``node`` or ``scene``
automatically at the start of the game, outside the current scene. 
You can also use it to create globally accessible objects. 
...
The next step is to register the scene as an Autoload. 
Starting from the menu, open **Project > Project Settings > Globals > Autoload**. 
Click the browse button and select ``music_player.tscn``, or by typing its path: ``res://music_player.tscn``. 
click the *Add* button on the right to register the node.

|image18|

With that, ``music_player.tscn`` now loads automatically into any scene you open or play.
