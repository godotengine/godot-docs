...
To create one, click on the ``AnimatedSprite2D`` you created, then look at the 
Inspector (normally on your right), open the ``Animation`` tab, then 
click on ``[empty]`` near ``Sprite Frames`` and click on ``New SpriteFrames``.
Click again on the new ``SpriteFrames`` you just created to open the 
``SpriteFrames`` panel:
...
them down. Click on the ``AnimatedSprite2D`` node, go to the Inspector, click on 
"Transform" under the the ``Node2D`` heading, and set the ``Scale`` property
to ``(0.5, 0.5)``.
