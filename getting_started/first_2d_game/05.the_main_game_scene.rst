...
After connecting the ``timeout()`` signal for each Timer nodes, Your scene should look like below (see connection icon next to each node):

.. image:: img/timer_signal.webp
