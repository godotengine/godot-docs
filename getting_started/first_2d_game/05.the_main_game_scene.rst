...
*Note: the reason we are using Node, and not a Node2D, is because this node will be a container for handling game logic and does not itself require 2D functionality.*

Click the "Instance" button and select your
saved ``Player.tscn``.
