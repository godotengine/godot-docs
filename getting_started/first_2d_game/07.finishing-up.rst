...
.. note:: If you already run the game with some stereo output, you have probably
        noticed the game sound being considerably balanced to the left. This is because 
        the ``AudioStreamPlayer`` are on the upper-left corner (position 0, 0), so place
        them in the center of the screen (240, 360) and the audio should sound balanced.
