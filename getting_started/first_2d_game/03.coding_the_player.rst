...
Click on *Project -> Project Settings* to open the project settings and then
click on the *Input Map* tab at the top. Enter "move_right" and then click
the "Add" button to add the ``move_right`` action, as shown here:

.. image:: img/input-mapping-add-action.png

Now we need to add a key assignment to this action. Press the "+" button and
then press "Key" in the pop-up menu.  A dialog will ask you to press the key
you want mapped.  Press the right arrow and then click "Ok":

.. image:: img/input-mapping-add-key.png

Repeat these steps to add three more mappings: ``move_left`` mapped to the left arrow
key, ``move_up`` mapped to the up arrow key, and ``move_down`` mapped to the down arrow key.
It should look something like this:

.. image:: img/input-mapping-completed.png

Press "Close" to close the project settings.

.. note::

  You can map multiple keys, joystick buttons, or joystick
  axes to the same input action.
