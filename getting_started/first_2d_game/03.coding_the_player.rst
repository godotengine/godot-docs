...
We may set the value of the first variable, "speed," in the Inspector by using the "export" keyword. This is useful for values that you wish to be able to alter like the built-in characteristics of a node. The property now displays in the "Script Variables" section of the Inspector when you click on the "Player" node. Remember that changing this value overrides the value written in the script.

.. warning:: You must (re)build the project assembly everytime you wish to see new export variables or signals if you're using C#. This build can be manually triggered by revealing the Mono Panel by clicking the word "Mono" at the bottom of the editing window, then clicking the "Build Project" button.
...
First, we must determine whether or not the player is pressing a key. We have four direction inputs to verify in this game. Input actions are defined under "Input Map" in the Project Settings. Custom events can be defined here, and different keys, mouse events, or other inputs can be assigned to them. We'll just utilise the default events like "ui right" and "ui left" that are associated to the arrow keys on the keyboard for this game.
