...
.. note::
    ...
    from the :menu:`Language` drop down menu before hitting create.
...
.. tip::
    ...
    length*, which is the amount of time that the previous frame took to complete.
...
signal will be emitted when a body touches the player. Click :button:`Connect...` and
the :ui:`Connect a Signal` window appears.

Godot will create a function with that exact name directly in the script
