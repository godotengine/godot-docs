...
- **FORMAT_PVRTC1_2** = **25** --- Texture format used on PowerVR-supported mobile platforms, uses 2-bit color depth with no alpha. More information can be found `here <https://en.wikipedia.org/wiki/PVRTC>`_.

**Note:** When creating an :ref:`ImageTexture<class_ImageTexture>`, an sRGB to linear color space conversion is performed.

- **FORMAT_PVRTC1_2A** = **26** --- Same as :ref:`FORMAT_PVRTC1_2<class_Image_constant_FORMAT_PVRTC1_2>`, but with an alpha component.

- **FORMAT_PVRTC1_4** = **27** --- Texture format used on PowerVR-supported mobile platforms, uses 4-bit color depth with no alpha. More information can be found `here <https://en.wikipedia.org/wiki/PVRTC>`_.
