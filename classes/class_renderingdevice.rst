...
.. XML source: https://github.com/godotengine/godot/tree/4.0/doc/classes/RenderingDevice.xml.
...
Returns ``true`` if the compute pipeline specified by the ``compute_pieline`` RID is valid, ``false`` otherwise.

\ **Note:** The ``compute_pieline`` parameter should be named "compute_pipeline", but this typo was kept in the 4.0.x branch to preserve compatibility for C# and GDExtension.
