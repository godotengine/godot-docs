...
Sets the size of the array. If the array is grown, reserves elements at the end of the array. If the array is shrunk, truncates the array to the new size. Calling :ref:`resize()<class_PackedColorArray_method_resize>` once and assigning the new values is faster than adding new elements one by one. Calling :ref:`resize()<class_PackedColorArray_method_resize>` returns the error value (ie: OK) not the new size of the array, use the .size() operator to find the size of the array.
