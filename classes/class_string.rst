...
Some string methods have corresponding variations. For example, methods that perform string comparison are **case-sensitive** unless stated otherwise, but variations suffixed with ``n`` (:ref:`countn()<class_String_method_countn>`, :ref:`findn()<class_String_method_findn>`, :ref:`replacen()<class_String_method_replacen>`, etc.) are **case-insensitive** and make no distinction between uppercase and lowercase letters. Another example is methods prefixed with ``r`` (:ref:`rfind()<class_String_method_rfind>`, :ref:`rsplit()<class_String_method_rsplit>`, etc.), which start from the end of the string instead of the beginning.
