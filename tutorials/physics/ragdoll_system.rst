...
Like many other features in the engine, there is a node to set up a ragdoll: the :ref:`PhysicalBone <api:class_PhysicalBone>` node. To simplify the setup, you can generate ``PhysicalBone`` nodes with the "Create physical skeleton" feature in the skeleton node.
