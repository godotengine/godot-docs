...
.. figure:: img/2d_platformer_demo.webp
   ...
   2D Platformer Demo accessible on Asset Library.
...
interfaces.
You can use the workspace selector located Godot's top edge at the middle, or choose a 2D 
...
opened scene or create a new one using the plus (+) button. The left and right docks should 
...
You can drag and drop compatible nodes from the FileSystem dock to add them into the 
viewport as nodes.
Dragging and dropping it adds the dragged node as a sibling node of the selected node 
(if the root node is selected, adds as a child).
Keeping :kbd:`Shift` pressed when dropping adds it as a child node of the selected node.
Holding :kbd:`Alt` when dropping adds the node as a child of the root node.
If :kbd:`Alt + Shift` is held during dropping, the node type can be selected if 
applicable.
...
- **Select Mode** (:kbd:`Q`): Allows selection of nodes in the viewport. Left clicking on a node 
  ...
  will not use the snapping options if snapping enabled.
...
- **Pan Mode**: Allows you to navigate in the viewport without accidentally selecting any nodes.
  You can also use :kbd:`G` (or :kbd:`Space`) and drag with left mouse button to do the same.
- **Ruler Mode**: After enabling, click on the viewport to display the current global 
  ...
  :kbd:`R` key also activates the ruler. If snapping is enabled, it also displays the 
...
- **Use Scale Snap**: Toggles snapping for scaling using the configured scaling step setting.
...
- **Configure Snap**: Opens the window shown above, offering a set of snapping parameters.
  ...
  - Grid Step: The distance between each grid in pixels. ``x`` and ``y`` can separately be adjusted.
  ...
  - Rotation Step: Defines the snapping degree. E.g., 15 means the node will rotate and snap 
    at each 15 degrees if rotation snap is enabled and the rotate mode is used.
...
- **Lock** (:kbd:`Ctrl + L`) the selected nodes, preventing selection and movement in the 
...
- **View** menu: Provides options to change view options of the viewport. Since its options 
...
Sprite2D is selected. This menu provides some quick actions or tools to
work on a specific node or selection. For example, while drawing a polygon, it will
provide buttons to add, modify, or remove points.
...
In the viewport, you will see two lines in two colours going across the screen infinitely: 
...
Alternatively, :kbd:`G` or :kbd:`Space` keys can be used.
If you enable `Editor Settings > Editors > Panning > Simple Panning`, you can activate
directly panning with :kbd:`Space` key only, without requiring dragging.
...
The black bars at the viewport's left and top are the **rulers**. You can use them to 
orient yourself in the viewport.
By default, the rulers will display the pixel coordinates of the viewport, numbered at 
each 100 pixels. Changing zoom factor will also increase or decrease the scaling of these 
numbers.
...
If you have at least one node in the scene, clicking on the black area of the horizontal or 
vertical ruler and dragging the mouse toward the viewport will create a purple guide, also showing 
its global position in the other axis. Releasing the mouse will leave it in that position. If you use 
the gray square located at the intersection of both the horizontal and vertical rulers, you will 
create a horizontal and a vertical line at the same time. Horizontal guides can be repositioned 
by dragging them inside the vertical ruler. Once you hover your mouse over a line inside the 
vertical ruler, your cursor will change. Similarly, vertical guides can be repositioned by 
dragging inside the horizontal ruler. To remove a guide, move it all the way back to the 
respective ruler.

You can also enable snapping to the created guides using `Smart Snap` menu.
...
- **Show Helpers**: Toggles the temporary display of an outline of the node, with the previous 
  ...
  initiated. For the `Control` nodes, it also shows the sizing parameters. Useful to see the deltas.
...
for everything GUI. For the 3D, the Godot uses the :ref:`Node3D <class_Node3D>` node.
