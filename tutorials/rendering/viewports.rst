...
- Rendering dynamic textures (more flexible than
  :ref:`drawable textures <doc_drawable_textures>`, but with greater overhead)
...
closest parent :ref:`class_Viewport` (going towards the root).
...
:ref:`class_Viewport` and it will draw ``MeshA``. ``CameraB``
will be captured by the :ref:`class_SubViewport` along with ``MeshB``.
