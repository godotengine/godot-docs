...
is only effective when the main scene is set to the scene in question, or when
...
Output formats are provided by the MovieWriter class.
Godot has 2 built-in MovieWriters, and more can be implemented by extensions:
...
If you chose to record a PNG image sequence with a WAV file besides it,
