...
Even though we may add new types and interface function with each minor release of Godot, we
...
- ``kind`` (required): The "kind" of type.
...
by their ``name`` using the ``GDExtensionInterfaceGetProcAddress`` function pointer, which is
...
- ``legacy_type_name``: The legacy name used for the function pointer type in the header generated
  ...
  this field only exists so that we can generate the header in a way that is backwards compatible
