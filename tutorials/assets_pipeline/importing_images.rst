...
When enabled, you may also adjust **Mipmaps > Alpha Test Threshold** to tune
the look of the texture at a distance. Higher values will preserve opacity
more aggressively at greater distances. In most cases, it's recommended to keep this
to its default value of ``0.5``, even if the material's
**Alpha Scissor Threshold** is set to a value other than ``0.5``
(the values do not have to match).
