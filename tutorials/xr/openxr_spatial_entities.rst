...
plane tracking and anchors. These are refered to as spatial capabilities.
...
.. list-table:: Spatial entity settings
   ...
   * - Enabled
     - Enables the core of the spatial entities system. This must be enabled for any of the spatial
       entities system to work.
   * - Enable spatial anchors
     - Enables the spatial anchors capability that allow use to create and track spatial anchors.
   ...
   * - Enables our built-in marker detection logic, this will automatically react to new markers being
       found or markers being moved around the players space.
...
When spatial entities are detected or created a
...
- Wait until the tracking status changes to ``OPENXR_SPATIAL_ENTITY_TRACKING_STATE_TRACKING``
...
set. It is this difference in flow that allows us to correctly react to new and existing
persistent anchors.

.. note::
    If you remove the persistence of an anchor, the UUID is destroyed but the anchor is not
    removed automatically. You will need to react to the completion of making an anchor
...
subscene as meta data for our UUID.
...
.. code-block:: gdscript
    ...
    func _on_spatial_tracking_state_changed(new_state) -> void:
        if new_state == OpenXRSpatialEntityTracker.OPENXR_SPATIAL_ENTITY_TRACKING_STATE_TRACKING and not made_persistent:
            ...
            # tracked scene.
            OpenXRSpatialAnchorCapability.make_anchor_persistent(anchor_tracker, RID(), Callable())
    ...
    func _on_uuid_changed() -> void:
        if anchor_tracker.uuid != "":
            ...
            if child_scene:
                ...
                # If we do not, lookup the UUID in our stored cache.
                ...
                if scene_path.is_empty():
                    ...
                    # Load a default scene so we can atleast see something.
...
With our anchor scene in place we can add a couple of function to our spatial manager script
...
.. code-block:: gdscript
    ...
    ## Create a new spacial anchor with the associated child scene
    ## If persistent anchors are supported, this will be created as a persistent node
    ## and we will store the child scene path with the anchors UUID for future recreation.
    func create_spacial_anchor(p_transform : Transform3D, p_child_scene_path : String):
        ...
        if not anchor_scene is OpenXRSpatialAnchor3D:
            push_error("Anchor scene for %s is not a OpenXRSpatialAnchor3D scene, has the manager been configured with an applicable anchor scene?" % [ new_anchor.name ])
            return
    ...
    ## If the spatial anchor is persistant, the associated UUID will be cleared.
    func remove_spacial_anchor(p_anchor : XRAnchor3D):
        # Do we have anchor support?
        if not OpenXRSpatialAnchorCapability.is_spatial_anchor_supported():
            push_error("Spatial anchors are not supported on this device!")
            return
        ...
        if tracker and tracker is OpenXRAnchorTracker:
            ...
            if anchor_tracker.has_uuid() and OpenXRSpatialAnchorCapability.is_spatial_persistence_supported():
                ...
                OpenXRSpatialAnchorCapability.make_anchor_unpersistent(anchor_tracker, RID(), _on_unperistent_complete)
            else:
                # Else we can just remove it.
    ...
    func _on_unperistent_complete(p_tracker : XRTracker):
...
the players vicinity. This data could come from a room capture performed by the user at
...
The root node for this scene must be a :ref:`XRAnchor3D<class_XRAnchor3D>` node.
...
Alternatively we can assign a material that will visualise the plane for debugging.
...
If supported by the XR runtime there is additional meta data you can query on the plane tracker
object. 
...
with a :ref:`XRAnchor3D<class_XRAnchor3D>` node at the root, save this and assign it to the
spatial manager as the scene to instantiate for markers.

The marker tracker should be fully configured when assigned so all that is neaded is a
    ...
    func _ready():
        ...
        if marker_tracker:
            match marker_tracker.marker_type:
                OpenXRSpatialComponentMarkerList.OPENXR_MARKER_TYPE_QRCODE:
                    ...
                    if data.type_of() == TYPE_STRING:
                        ...
                        pass
                OpenXRSpatialComponentMarkerList.OPENXR_MARKER_TYPE_MICRO_QRCODE:
                    var data = marker_tracker.get_marker_data()
                    if data.type_of() == TYPE_STRING:
                        # Data is a QR code as a string, usually a URL
                        pass
                        ...
                        # Data is binary, can be anything
                        pass
                OpenXRSpatialComponentMarkerList.OPENXR_MARKER_TYPE_ARUCO:
                    # Use marker_tracker.marker_id to identify the marker
                    pass
                OpenXRSpatialComponentMarkerList.OPENXR_MARKER_TYPE_APRIL_TAG:
                    # Use marker_tracker.marker_id to identify the marker
                    pass

As we can see, QR Codes provide a data block that is either a string or byte array.
...
**Maybe see if we can work out atleast one example here, need access to hardware that supports this, should have that soon**
...
Spatial contexts allow us to configure how we interact with one of more capabilities.
...
access.
Each capability will enable those components we support for that capability.
Settings can determine which components will be enabled.
We'll look at these configuration objects in more detail as we look at each supported capacity.
...
.. code-block:: gdscript
    ...
    func _enter_tree():
        ...
        if openxr_interface and openxr_interface.is_initialized():
            ...
            openxr_interface.session_begun.connect(_setup_spatial_context)
    ...
    func _exit_tree():
        ...
        var openxr_interface : OpenXRInterface = XRServer.find_interface("OpenXR")
        if openxr_interface and openxr_interface.is_initialized():
            openxr_interface.session_begun.disconnect(_setup_spatial_context)
...
OpenXR issues an event when there are new entities to be processed, this results in the
``spatial_discovery_recommended`` signal being issued by our
:ref:`OpenXRSpatialEntityExtension<class_OpenXRSpatialEntityExtension>` singleton
...
.. code-block:: gdscript
    ...
    func _on_perform_discovery(p_spatial_context):
        ...
        discovery_result = OpenXRSpatialEntityExtension.discover_spatial_entities(spatial_context, [ \
            OpenXRSpatialEntityExtension.OPENXR_SPATIAL_COMPONENT_TYPE_BOUNDED_2D, \
            OpenXRSpatialEntityExtension.OPENXR_SPATIAL_COMPONENT_TYPE_PLANE_ALIGNMENT \
        ])
...
atleast one of the specified components.
...
Generally speaking you would only perform update snapshots when its likely entities
change or have a lifetime process. A good example of this are persistent anchors and
markers. Consult the documentation around a capability to determine if this is needed.
...
are different between your discovery snapshot and/or your update snapshot, you have to take into account
that you query different components.

Querying snapshots
""""""""""""""""""

Once we have a snapshot we can run queries over those snapshots to obtain the data held within.
    ...
    func _process_snapshot(p_snapshot):
        ...
        if OpenXRSpatialEntityExtension.query_snapshot(p_snapshot, [ query_result_data, bounded2d_list, alignment_list]):
            for i in query_result_data.get_entity_id_size():
                ...
                if entity_state == OpenXRSpatialEntityTracker.OPENXR_SPATIAL_ENTITY_TRACKING_STATE_STOPPED:
                    ...
                    if entity_state == OpenXRSpatialEntityTracker.OPENXR_SPATIAL_ENTITY_TRACKING_STATE_TRACKING:
                        ...
                        # hold further data. You should subclass this class to store the additional
...
.. note::
    In the above example we're relying on ``OPENXR_SPATIAL_ENTITY_TRACKING_STATE_STOPPED`` to clean up
...
entity is not cleaned up early. This is a prerequisit for performing an update query on
this entity.
...
Both functions return a RID that we can use in further functions that require our entity object.
...
As we shown in our example before we always start with creating a spatial context but now using the
:ref:`OpenXRSpatialCapabilityConfigurationAnchor<class_OpenXRSpatialCapabilityConfigurationAnchor>`
configuration object.
We'll show an example of this code after we discuss persistance scopes.
...
In order to enable making anchors persistent you also have to setup a persistence scope.
...
.. list-table:: Persistence scopes
   ...
   * - Enum
     - Description
   * - OPENXR_SPATIAL_PERSISTENCE_SCOPE_SYSTEM_MANAGED
     - Provides the application with read-only access (i.e. application cannot modify this store)
   ...
   * - OPENXR_SPATIAL_PERSISTENCE_SCOPE_LOCAL_ANCHORS
     - Persistence operations and data access is limited to spatial anchors, on the same device,
       for the same user and same app (using `make_anchor_persistent` and
       `make_anchor_unpersistent` functions)
...
.. code-block:: gdscript
    ...
    func _setup_persistence_context():
        # Already setup?
        ...
        var scope : int = 0
        if OpenXRSpatialAnchorCapability.is_persistence_scope_supported(OpenXRSpatialAnchorCapability.OPENXR_SPATIAL_PERSISTENCE_SCOPE_LOCAL_ANCHORS):
            scope = OpenXRSpatialAnchorCapability.OPENXR_SPATIAL_PERSISTENCE_SCOPE_LOCAL_ANCHORS
        elif OpenXRSpatialAnchorCapability.is_persistence_scope_supported(OpenXRSpatialAnchorCapability.OPENXR_SPATIAL_PERSISTENCE_SCOPE_SYSTEM_MANAGED):
            scope = OpenXRSpatialAnchorCapability.OPENXR_SPATIAL_PERSISTENCE_SCOPE_SYSTEM_MANAGED
        else:
            # Don't have a known persistence stscopeore, report and just setup without it.
        ...
        if not future:
            # Couldn't create persistence scope? Just setup without it.
            _setup_spatial_context()
            return
    ...
    func _enter_tree():
        ...
        if openxr_interface and openxr_interface.is_initialized():
            ...
            openxr_interface.session_begun.connect(_setup_persistence_context)

            # And in case it is already up and running, call it already,
            # it will exit if we've called it too early.
            _setup_persistence_context()
    ...
    func _exit_tree():
        ...
        var openxr_interface : OpenXRInterface = XRServer.find_interface("OpenXR")
        if openxr_interface and openxr_interface.is_initialized():
            openxr_interface.session_begun.disconnect(_setup_persistence_context)
...
.. code-block:: gdscript
    ...
    func _setup_spatial_context():
        ...
        # Not supported or we're not yet setup.
        if not OpenXRSpatialAnchorCapability.is_spatial_anchor_supported():
            return
        ...
        # And setup our persistence configuration object (if needed)
...
We also want to perform regular update queries, here we are only interested in state so we do want to make
...
.. list-table:: Anchor components
   ...
   * - Component
     - Data class
     - Description
   * - OPENXR_SPATIAL_COMPONENT_TYPE_ANCHOR
   ...
   * - OPENXR_SPATIAL_COMPONENT_TYPE_PERSISTENCE
...
.. code-block:: gdscript
    ...
    func _on_perform_discovery(p_spatial_context):
        ...
        # Perform our discovery
        discovery_result = OpenXRSpatialEntityExtension.discover_spatial_entities(spatial_context, [ \
            OpenXRSpatialEntityExtension.OPENXR_SPATIAL_COMPONENT_TYPE_ANCHOR, \
            OpenXRSpatialEntityExtension.OPENXR_SPATIAL_COMPONENT_TYPE_PERSISTENCE \
        ])
        ...
        if OpenXRSpatialEntityExtension.query_snapshot(p_snapshot, result_data):
            for i in query_result_data.get_entity_id_size():
                ...
                if entity_state == OpenXRSpatialEntityTracker.OPENXR_SPATIAL_ENTITY_TRACKING_STATE_STOPPED:
                    ...
                    # Only if we're tracking, we update our position.
                    if entity_state == OpenXRSpatialEntityTracker.OPENXR_SPATIAL_ENTITY_TRACKING_STATE_TRACKING:
                    ...
                    # But persistence data is be big exception, it can be provided even if we're not tracking.
                    if p_get_uuids:
...
There is no need for update snapshots here, we just do our discovery snapshot and implement our process function.
...
.. list-table:: Plane tracking components
   ...
   * - Component
     - Data class
     - Description
   * - OPENXR_SPATIAL_COMPONENT_TYPE_BOUNDED_2D
   ...
   * - OPENXR_SPATIAL_COMPONENT_TYPE_PLANE_ALIGNMENT
   ...
   * - OPENXR_SPATIAL_COMPONENT_TYPE_MESH_2D
   ...
   * - OPENXR_SPATIAL_COMPONENT_TYPE_POLYGON_2D
   ...
   * - OPENXR_SPATIAL_COMPONENT_TYPE_PLANE_SEMANTIC_LABEL
...
Our plane tracking configuration object already enables all supported components but we'll need to interogate
...
.. code-block:: gdscript
    ...
    func _process_snapshot(p_snapshot):
        ...
        for entity_id in org_entities:
            var entity_tracker : OpenXRPlaneTracker = entities[entity_id]
            entity_tracker.spatial_tracking_state = OpenXRSpatialEntityTracker.OPENXR_SPATIAL_ENTITY_TRACKING_STATE_STOPPED
            XRServer.remove_tracker(entity_tracker)
            entities.erase(entity_id)
...
Marker tracking works similar to plane tracking however we're now tracking specific entities in
...
.. code-block:: gdscript
    ...
    func _setup_spatial_context():
        ...
        if not OpenXRSpatialMarkerTrackingCapability.aruco_is_supported():
            ...
            aruco_config.aruco_dict = OpenXRSpatialCapabilityConfigurationAruco.OPENXR_SPATIAL_MARKER_ARUCO_DICT_7X7_1000
...
Every marker regardless of the type of marker will consist of two components:
...
Finally we add our discovery implementation:
...
.. code-block:: gdscript
    ...
    func _on_perform_discovery(p_spatial_context):
        ...
        # Perform our discovery
        discovery_result = OpenXRSpatialEntityExtension.discover_spatial_entities(spatial_context, [\
            OpenXRSpatialEntityExtension.OPENXR_SPATIAL_COMPONENT_TYPE_MARKER, \
            OpenXRSpatialEntityExtension.OPENXR_SPATIAL_COMPONENT_TYPE_BOUNDED_2D \
        ])
