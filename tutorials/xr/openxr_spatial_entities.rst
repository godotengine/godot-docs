...
are different between your discovery snapshot and your update snapshot, you have to take into account
...
.. note::
    ...
    For capabilities that only rely on discovery snapshots you may wish to do a clean up based on
...
.. list-table:: Persistence scopes
   ...
   * - PERSISTENCE_SCOPE_LOCAL_ANCHORS
     - Persistence operations and data access is limited to spatial anchors, on the same device,
       for the same user and app (using `make_anchor_persistent` and
