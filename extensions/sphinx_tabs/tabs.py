...
def add_assets(app, pagename, templatename, context, doctree):
    ...
    css_files = [
        posixpath.join("_static", path) for path in assets if path.endswith("css")
    ]
    script_files = [
        posixpath.join("_static", path) for path in assets if path.endswith("js")
    ]
...
def copy_assets(app, exception):
    ...
    builders = ("html", "readthedocs", "readthedocssinglehtmllocalmedia", "singlehtml")
    if app.builder.name not in builders:
        app.warn(
            "Not copying tabs assets! Not compatible with %s builder" % app.builder.name
        )
        return
    if exception:
