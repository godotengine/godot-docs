...
it is not strictly required to use an engine build that contains debugging symbols to get
useful graphics debugging information. However, using a
:ref:`development build <doc_introduction_to_the_buildsystem_dev_build>` compiled with the
``dev_build=yes`` SCons option is strongly recommended. Non-development builds
strip much of the debug information that GPU debuggers can make use of, such as rendering
markers.
