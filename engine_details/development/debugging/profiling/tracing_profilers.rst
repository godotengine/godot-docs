...
Godot currently supports two tracing profilers: Tracy and Perfetto.
...
time of this writing) using Git:
...
However, if you're on Linux or MacOS, you'll need to build it yourself.
...
If it builds successfully, the ``tracy-profiler`` executable (or ``tracy-profiler.exe``
on Windows) will be in ``profiler/build``.

Record a trace
~~~~~~~~~~~~~~

First, export your game using the release templates you built above, and run it.

Then, launch the Tracy server - you'll see something like this:

.. image:: img/cpp_profiler_tracy_start.webp

When you're ready to record a trace (for example, when you've hit the part of
your game that is exhibiting performance issues), press the "Connect" button.

You'll immediately see tracing data start coming in:
...
You'll probably need to zoom a bit before your able to see the individual
events from Godot.
